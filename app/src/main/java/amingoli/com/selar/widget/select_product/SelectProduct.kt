...
class SelectProduct (context: Context?, attrs: AttributeSet?) : LinearLayout(context, attrs) {
    ...
    private var listProducts = ArrayList<Product>(App.database.getAppDao().selectProduct(App.branch()))
    private var listCategory = ArrayList<Category>(App.database.getAppDao().selectUnderCategory(App.branch(),0))
...
//        Test
        ic_back.setOnClickListener {
            if (!listCategoryForBack.isNullOrEmpty()){
                ...
                adapterTagList?.updateList(App.database.getAppDao().selectUnderCategory(App.branch(),listCategoryForBack[pos]))
                if (listCategoryForBack[pos] == 0){
                    adapterProduct?.updateList(App.database.getAppDao().selectProduct(App.branch()))
                }else adapterProduct?.updateList(App.database.getAppDao().selectProductByCategory(App.branch(), listCategoryForBack[pos]))
...
//                Test
                ...
                adapterProduct?.updateList(App.database.getAppDao().selectProductByCategory(App.branch(), item.id!!))
